...
class CouponSetupRepository extends BaseRepository implements CouponSetupRepositoryInterface
{
    ...
    public function getUserCouponList(array $data, $limit = null, $offset = null)
    {
        $model = $this->model
            ...
            ->when($data['category_type'] && ($data['category_type'] != 'parcel' && $data['category_type'] != 'all') , function ($query) use ($data) {
                $query->whereHas('vehicleCategories', function ($query1) use ($data) {
                    $query1->where('id', $data['category_type']);
                });
            })
            ->when($data['category_type'] && $data['category_type'] == 'parcel', function ($query) {
                $query->where(function ($query){
                    $query->whereJsonContains('category_coupon_type', 'parcel')->orWhereJsonContains('category_coupon_type', 'all');
                });
            })
