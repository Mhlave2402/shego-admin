...
class UserLastLocationRepository extends BaseRepository implements UserLastLocationRepositoryInterface
{
    ...
    public function getNearestDrivers($attributes): mixed
    {
        return $this->lastLocation
            ...
            ->when(array_key_exists('has_baby_seat', $attributes) && $attributes['has_baby_seat'], function ($query) {
                $query->whereHas('user.vehicle', fn($query) => $query->where('has_baby_seat', true));
            })
            ->when(array_key_exists('gender', $attributes) && $attributes['gender'], function ($query) use ($attributes) {
