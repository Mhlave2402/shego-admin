    ...
    public function requestAction(Request $request): JsonResponse
    {
        ...
        if (!$this->tripRequestService->canDriverAcceptRegularTrip(user: $user, trip: $trip, estimatedDistance: $estimatedDistance, avgKmPerMinute: $avgKmPerMinute)) {
            ...
            return response()->json(responseFormatter(constant: [
                ...
                'message' => translate(key: 'You have a scheduled trip in :timeDifference minutes', replace: ['timeDifference' => $scheduledAt->diffInMinutes(now())], locale: $trip->driver?->current_language_key),
            ]), 403);
        }
        ...
        if (!$this->tripRequestService->canDriverAcceptScheduledTrip(user: $user, trip: $trip, estimatedDistance: $estimatedDistance, avgKmPerMinute: $avgKmPerMinute)) {
            ...
            return response()->json(responseFormatter(constant: [
                'response_code' => 'trip_cannot_be_accepted_403',
                'message' => now()->lt($scheduledAt) ? translate(key: 'You will not be able to reach the pickup location in :timeDifference minutes', replace: ['timeDifference' => $scheduledAt->diffInMinutes(now())], locale: $trip->driver?->current_language_key) : translate('Your current trip has not ended yet'),
            ]), 403);
        }
